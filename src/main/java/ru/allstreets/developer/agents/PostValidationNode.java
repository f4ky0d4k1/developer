    ...
    private String buildValidatorPrompt(AgentContext ctx, int reworkCount) {
        ...
        sb.append("""
                ...
                6. Отпишись о результате в задачу Трекера (issue ниже, если он есть): оставь комментарий
                   ...
                   комментарий уже есть — не дублируй, проверь и при необходимости поправь. Если для
                   работы с Трекером доступны навыки (skills) — используй их. Создаёт/связывает задачу
                   Трекера АНАЛИТИК — тебе нужно только отписаться в существующую, не создавай новую.
