...
- **Запуском управляет аналитик**: `analyst` ставит `nextStep=reporter` (`NextStep.REPORTER`) — это
  единственное исключение из «роутинг по флагам, не по `nextStep`», потому что текст в Трекере не код.
  Граф: `analyst → reporter → post_validation`; `analystGoesToPostValidation` явно исключает этот
  `nextStep`, чтобы reporter-задача не утекла в валидацию мимо репортёра.
