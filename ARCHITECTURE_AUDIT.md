...
## 39. Реакция на прямой ответ боту (reply), а не только на @mention

**Статус: DONE**

- В группах бот отбрасывал сообщения без `@mention` и без pending-вопроса, даже если это был reply на
  сообщение самого бота — пользователь отвечал на HITL-вопрос, а бот «не реагировал».
- Добавлен признак `isReplyToBot(msg, botUsername)` в `TelegramBotListener`: `reply_to_message` на сообщение
  бота (`from.is_bot` + username == `telegram.bot-username`) пропускается к `ConversationAgent` без `@mention`.
- Whitelist (`isChatAllowed`) и `trigger-users` (в `launch_task`) не тронуты — фильтры работают как раньше.

Тесты: `TelegramBotListenerReplyTest`.

## 40. Спека аналитика — markdown, а не сплошная строка

**Статус: DONE**

- Промпт аналитика дословно требовал `spec` «одной строкой» — модель выдавала сплошную простыню
  `ЗАДАЧА/НАЙДЕНО В КОДЕ/ДЕКОМПОЗИЦИЯ` без переносов, а в HITL-вопросе писала буквальный `\n`.
- `spec` — markdown с реальными переносами (`##`/`-`), запрет сплошной строки и заглавных меток;
  `clarificationQuestion` — нумерованный список с реальными переносами.
- Подстраховка `HumanLoopService.askHuman`: буквальный `\n` заменяется на перенос строки.

## 41. Задачи в одном чате идут параллельно

**Статус: DONE**

- Раньше `launch_task` прерывал running-задачу чата («одна RUNNING-задача на чат»). Убрано —
  `TaskMcpTools.launchTask` больше не трогает running-задачи; слотов 10, параллельность разрешена.
- Соответствие «сообщение → задача» решает `ConversationAgent` по контексту/истории/`getPendingQuestionsForChat`
  (map `taskId → вопрос`): `HITL_ANSWER` возвращает taskId того вопроса, на который отвечает сообщение.
- `resumeWithAnswer(taskId, …)` уже привязан к taskId (§34), а не к чату — параллельный resume не пересекается.

Тесты: `TaskMcpToolsLaunchTaskTest`.

## 42. Ретрай после зависания дополняет промпт предупреждением

**Статус: DONE**

- Stall-guard обрывал зависший ран, но ретрай запускал модель с ТЕМ ЖЕ промптом — она повторяла зависающий
  шаг (аналитик циклил bash/grep по репозиторию, 16 tool_calls, 300с без прогресса).
- `OpenCodeClient.withStallWarning`: при СВЕЖЕМ старте (не HITL-resume) ищет последний `ABORTED`-ран по
  `(taskId, agentName)` и дополняет промпт: причина зависания + «работай по-другому» (не повторять шаг,
  сужать область поиска, не запускать долгие команды).

Тесты: `OpenCodeClientStallWarningTest`.

## 43. Правки opencode-config не пересобирают образ сайдкара

**Статус: DONE**

- `opencode-config/agents/*.md` и `opencode.jsonc` МОНТИРУЮТСЯ в рантайме, а не запекаются в образ, но
  `OPENCODE_TAG` был завязан на `opencode-config` → каждая правка промптов пересобирала байт-в-байт тот же
  образ и пересоздавала контейнер (~20 мин даунтайма, молчаливое падение задач — инцидент 427edb3c/ad33e902).
- `deploy.yml`: `OPENCODE_TAG` ключится только на `opencode.Dockerfile`; `deploy.sh` после `up -d` делает
  `docker compose restart opencode` — правки промптов подхватываются за секунды без pull/recreate.
- Первый деплой один раз дотянет старый (существующий в реестре) `oc-` тег — дальше кэш.

## 44. ОТКРЫТО: деплой developer-контейнера роняет RUNNING-задачи (автовозобновление)

**Статус: ОТКРЫТО (спорно — решение не принято)**

- Проблема: `@PreDestroy shutdown()` прерывает потоки задач → граф возвращает error-результат →
  `handleResumeResult`/`runTask` метит задачу FAILED. После рестарта `CheckpointRecoveryListener` пропускает
  FAILED (чекпоинт сохранён, но автоподъёма нет) → задачи теряются, нужен ручной restart.
- Механизм автовозобновления уже есть: recovery умеет поднимать RUNNING-задачи из чекпоинта
  (`resumeAfterRestart`). Мешает только статус FAILED, который shutdown ставит ПОБОЧНО (из-за прерывания).
- **Вариант A (минимальный):** флаг `shuttingDown` (ставится в `shutdown()` до `cancel(true)`); при нём НЕ
  метить FAILED (оставить RUNNING) во всех местах `markFailed` по результату графа. После рестарта recovery
  поднимет задачу сам.
- **Вариант B (A + чище):** дополнительно при shutdown абортить in-flight OpenCode-прогоны (метить ABORTED в
  БД), иначе после рестарта `resumeOrStart` пойдёт опрашивать мертвую сессию сайдкара → stall (300с) → abort →
  ретрай с предупреждением (§42). Работает, но ~5 мин пауза на резюм.
- Нюанс B: HITL-пауза не страдает — ожидающая задача в `runningTasks` не числится, её pending живёт в БД.
- Сомнение/открытый вопрос: не теряется ли что-то при автоподъёме (WIP в worktree при оборванном git-вызове →
  возможный `.git/index.lock`/грязное дерево; см. обсуждение «слот упал → создать новый»). Решение НЕ принято.
